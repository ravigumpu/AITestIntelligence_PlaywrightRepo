...
test.describe("bulk generated suite 040", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 40 });
  ...
  test("increment updates count", async ({ page }) => {
    ...
    await expect(page.locator(selectors.count)).toHaveText("41");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-040");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-040");
  });
});
