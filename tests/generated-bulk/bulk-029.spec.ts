...
test.describe("bulk generated suite 029", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 29 });
    await expect(page.locator(selectors.count)).toHaveText("29");
  });

  test("increment updates count", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 29 });
    await page.locator(selectors.incrementButton).click();
    await expect(page.locator(selectors.count)).toHaveText("30");
  });

  test("decrement updates count", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 29 });
    await page.locator(selectors.decrementButton).click();
    await expect(page.locator(selectors.count)).toHaveText("28");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-029");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-029");
  });
});
