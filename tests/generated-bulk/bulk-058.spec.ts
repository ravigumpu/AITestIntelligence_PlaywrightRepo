...
test.describe("bulk generated suite 058", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 58 });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-058");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-058");
  });
});
