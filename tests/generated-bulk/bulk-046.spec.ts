...
test.describe("bulk generated suite 046", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 46 });
  ...
  test("decrement updates count", async ({ page }) => {
    ...
    await expect(page.locator(selectors.count)).toHaveText("45");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-046");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-046");
  });
});
