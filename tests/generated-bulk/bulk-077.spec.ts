...
test.describe("bulk generated suite 077", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 77 });
  ...
  test("increment updates count", async ({ page }) => {
    ...
    await expect(page.locator(selectors.count)).toHaveText("78");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-077");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-077");
  });
});
