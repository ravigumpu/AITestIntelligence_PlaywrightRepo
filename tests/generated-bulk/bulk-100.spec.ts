...
test.describe("bulk generated suite 100", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 100 });
    await expect(page.locator(selectors.count)).toHaveText("100");
  });

  test("increment updates count", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 100 });
    await page.locator(selectors.incrementButton).click();
    await expect(page.locator(selectors.count)).toHaveText("101");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-100");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-100");
  });
});
