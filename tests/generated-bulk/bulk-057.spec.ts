...
test.describe("bulk generated suite 057", () => {
  test("count starts at configured seed", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 57 });
    await expect(page.locator(selectors.count)).toHaveText("57");
  });

  test("increment updates count", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 57 });
    await page.locator(selectors.incrementButton).click();
    await expect(page.locator(selectors.count)).toHaveText("58");
  });

  test("decrement updates count", async ({ page }) => {
    await mountDemoPage(page, { initialCount: 57 });
    await page.locator(selectors.decrementButton).click();
    await expect(page.locator(selectors.count)).toHaveText("56");
  });
  ...
  test("add list item with deterministic value", async ({ page }) => {
    ...
    await page.locator(selectors.nameInput).fill("bulk-item-057");
    await page.locator(selectors.addItemButton).click();
    await expect(page.locator(selectors.items).first()).toHaveText("bulk-item-057");
  });
});
